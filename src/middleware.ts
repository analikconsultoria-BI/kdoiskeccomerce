...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/admin') && !pathname.startsWith('/admin/login')) {
    ...
    // Verificamos se há usuário. Assumimos que, se ele consegue logar, ele tem permissão de admin.
    // (Caso houvessem outras roles no futuro, verificaríamos `user.role === 'admin'`)
    if (!user) {
...
export const config = {
  matcher: ['/admin/:path*', '/api/bling/:path*', '/api/checkout/:path*'],
};
